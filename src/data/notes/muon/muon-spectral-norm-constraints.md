...
source:
  ...
  url: "https://github.com/J-shang/Muon/blob/ae2b5f9e6ee06b411aef2220e361c75988a7d753/%E8%AE%BA%E6%96%87%E7%B2%BE%E8%AF%BB/09-Spectral-Norm-Constraints.md"
  revision: "ae2b5f9e6ee06b411aef2220e361c75988a7d753"
  syncedAt: "2026-07-14"
  contentHash: "sha256:476858b900381335ecf68d11a299e407ea1b34eea9dd5c19dab1eb173e68154c"
  manifest: "muon"
  managed: true
---
> source: [arXiv:2506.15054](https://arxiv.org/abs/2506.15054)
> source class: 理论与实验预印本
> confidence: recurrence bound `verified`；KKT 收敛限于论文假设；大模型隐式正则解释 `plausible`
...
- `extends` → *Old Optimizer, New Norm* 的单步 direction 到含 momentum/decay 的动态。
- `uses` → nuclear/spectral duality。
- `does-not-imply` → weight 是半正交矩阵；约束的是最大奇异值上界。
- `approximation-gap` → 实际 Muon 用有限步 polynomial，不一定严格满足 exact matrix-sign properties。
...
| 本笔记 $\limsup\|W_t\|_2\le1/\lambda$ 递推 | §6.1 Proposition 1 的特化 | `仓库内展开证明`，与论文命题一致 |
| “大模型隐式正则解释 plausible” | §8.3只观察 singular spectrum/bounds | `谨慎综合`；因果泛化收益未证 |
| finite NS 可能使 bound 常数改变 | 论文只称实作是 high-order polynomial | `开放实现检查`，不是论文已完成的定理 |
